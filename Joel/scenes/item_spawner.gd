...
func spawn_item():
	...
	elif item_to_spawn.group == "items dropeables":
		var duplicate_res = item_to_spawn.duplicate()
		var new_topping = topping_scene.instantiate()
		add_child(new_topping)
		new_topping.global_position = spawn_pos.global_position + spawn_pos.pivot_offset
		new_topping.initialpos = new_topping.global_position
		new_topping.new_pos = new_topping.initialpos
		new_topping.set_info(duplicate_res)
...
func _on_spawn_area_area_entered(area: Area2D) -> void:
	if area.get_parent().is_in_group("item") or area.get_parent().is_in_group("servidores") or area.get_parent().is_in_group("salsa") or area.get_parent().is_in_group("items dropeables"):
